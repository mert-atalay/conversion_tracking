...
This started as a planning document. Later on 2026-05-04, Franchise USA GTM Version `16` was published to move Meta browser tracking to the USA dataset `1531247935333023`, Version `17` was published to pause old active USA micro/click tags, and Version `18` was published to add a Meta Lead reliability fallback. No GA4 or Google Ads account settings were changed by this update.
...
| Meta | Partial for USA via GTM and Events Manager UI/API | USA browser runtime now sends to dataset `1531247935333023` through GTM, and USA custom conversion `1915200622465036` / `USA Franchise Lead` was created. | Meta Events Manager receipt for live `Lead` still needs confirmation before optimization-event signoff. |
...
| Franchise USA | `6004334435` | `GTM-5LZMHBZL` | Container read/write/publish verified | USA helper/dispatch events, GA4 mapping, Meta dataset split, legacy micro/click cleanup, and Meta Lead fallback are configured through Version `18`; Ads final helper-event tags remain pending account/action decision. |
...
| Step | Status | Action | Output |
|---|---|---|---|
| 1 | Complete | Audit `GTM-5LZMHBZL` Version 15 helper/dispatch and GA4 mapping for USA forms; publish Version `16` for USA Meta dataset split; publish Version `17` to pause old active micro/click tags; publish Version `18` for Meta Lead reliability. | USA GTM mapping table plus Version `18` evidence. |
...
| 6 | Partially complete | USA moved to separate Meta dataset `1531247935333023` through GTM Version `16`, retained it through Version `18`, old shared WordPress pixel source was removed, and custom conversion `1915200622465036` / `USA Franchise Lead` was created. | Meta Events Manager receipt for live standard `Lead` remains. |

USA decision to make:

- USA has now been separated at the browser/GTM runtime layer with dataset `1531247935333023`; `USA Franchise Lead` custom conversion exists, but live Meta Events Manager receipt and campaign optimization selection still need confirmation before treating it as fully signed off.
...
| Gap | Priority | Needed |
|---|---:|---|
| Meta Events Manager/API | High | USA custom conversion is created; still need parent/Canada dataset checks plus USA live Events Manager receipt and optimization-event confirmation. |
