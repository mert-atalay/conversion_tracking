...
| Platform settings changed | Yes: USA GTM Version `16` was published to move Meta to USA dataset `1531247935333023`; USA GTM Version `17` was published to pause old active micro/click tags; USA GTM Version `18` was published to add a Meta Lead reliability fallback. Meta custom conversion `1915200622465036` / `USA Franchise Lead` was created. No GA4 or Google Ads account settings changed. |
...
| Franchise USA GTM runtime | Verified | `GTM-5LZMHBZL` Version `18` contains the helper/dispatch path, `G-YL1KQPWV0M`, `AW-11088792613`, and Meta dataset `1531247935333023`; old active GA4/GAds/Meta click tags were paused and Meta `Lead` tags have an `fbq` init fallback. | Confirm Meta Events Manager receipt; decide USA Google Ads final submit action before adding Ads final helper tags. |
...
| Franchise USA Meta | Partial | USA dataset `1531247935333023` is now live through GTM Version `18`; old shared pixel `918227085392601` was removed from active USA WordPress header/body options; post-purge/runtime checks saw `1531247935333023` and zero old-dataset runtime occurrences; custom conversion `1915200622465036` / `USA Franchise Lead` exists. | Confirm Events Manager receipt for live standard `Lead` and select the campaign optimization event. |
...
Remaining Meta item: Events Manager must still be checked directly for live `Lead` receipt on dataset `1531247935333023`.
...
## 2026-05-04 Franchise USA GTM Version 18 And Meta Custom Conversion

What changed:

- USA GTM container `GTM-5LZMHBZL` was published as Version `18`: `CEFA Franchise USA Meta Lead reliability fix - 2026-05-04`.
- Tags `267` and `268` still send Meta standard `Lead` to dataset `1531247935333023`, but now include an `fbq` init fallback.
- Meta custom conversion `1915200622465036` / `USA Franchise Lead` was created under ad account `act_505300888223754`.
- The custom conversion uses standard `Lead` plus URL path containing `inquiry-thank-you`, scoped to USA dataset `1531247935333023`.

Remaining Meta item: confirm that the next real USA Form `1` thank-you submission appears as `Lead` in Meta Events Manager for dataset `1531247935333023`.
...
| 4 | Confirm USA Meta Events Manager live `Lead` receipt for dataset `1531247935333023`. | Pending | USA is now separated in GTM/browser runtime and custom conversion `1915200622465036` / `USA Franchise Lead` exists; Meta UI receipt/signoff is still needed. |
