#!/usr/bin/env python3
"""Read-only GreenRope phase and phase-path dictionary snapshot."""
...
def main() -> None:
    ...
    print(json.dumps({"phases": [asdict(row) for row in client.phases()], "phase_paths": [asdict(row) for row in client.phase_paths()]}))
