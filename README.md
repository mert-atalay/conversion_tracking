...
- Parent `cefa.ca` is live on plugin `0.6.1` with Attribution Bridge and ledger shadow modes; confirmed `school_inquiry_submit` ownership remains unchanged across GA4, Google Ads, and Meta. The ledger uses an opaque HttpOnly cookie and refreshed signed form fallback without replacing Form 4, School Manager, Synuma, or existing compatibility fields.
