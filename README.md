...
- Franchise USA `franchisecefa.com` now renders the WPCode fallback bridge, has verified Form `1` `franchise_inquiry_submit` and Form `2` `real_estate_site_submit` dataLayer events, and has live GTM Version `17` helper-event mapping / Meta USA dataset cleanup through `GTM-5LZMHBZL`.
